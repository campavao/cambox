...
class App extends React.Component {
  ...
  getServerList = async () => {
    return await fetch(`/getList/?code=${this.state.access}`)
  ...
  signOutMessage = () => {
    return (
      <Button onClick={this.signOut} variant="link" className="signout">
        sign out
      </Button>
    );
  };
      ...
      <Container>
        {isPhone ? (
          <WordUser
            ...
            SignoutMessage={this.signOutMessage}
          />
        ) : (
          <div>
        ...
        {access != null && serverList.length > 0 && (
          <WordViewer list={serverList} access={access} />
        )}
      </Container>
    );
  }
